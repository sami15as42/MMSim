	...
	function Heap() {
		this.head = null;
		this.size = 0;
		// Allouer un espace mémoire au processus en utilisant la méthode BuddySystem
		this.requestAllocation = async function(process) {
			var instructions=document.getElementById("ins");
			block = this.head;
	            while ((block.size < process.size) || (!block.available)) {//Chercher le premier bloc pouvant contenir le processus
				block = block.next;
				if (block== null) 
				{
					if (!process.traiter)
					{
						...
						instructions.innerHTML="Chercher dans MC le premier bloc disponible dont la taille est une puissance de 2 >= à "+process.size+" Ko";	
						await sleep(5000);
						instructions.innerHTML="Aucun bloc n'est disponble avec ce critère";
			...
			instructions.innerHTML="Chercher dans MC le premier bloc disponible dont la taille est une puissance de 2 >= à "+process.size+" Ko";	
			await sleep(5000);
			instructions.innerHTML="Il existe un bloc dont la taille est  "+block2.size+" Ko";
			this.repaint1(block2);
			block1 = block2;
			await sleep(3000);
			this.repaint();
			instructions.innerHTML="Subdivision successive du block trouvé en 2 jusqu'à obtention d'un bloc ayant une taille proche de "+process.size+" Ko";
			...
			instructions.innerHTML="Allocation du bloc";
			await sleep(3000);  
		...
		if (!enCours)
		{
			for (i=0; i<processes.length; i++) {
				...
				if (!process.isAllocated()) {
					...
					heap.fusionner();
					process.tick();
					if (process.timeLeft < 1) {//Liberer l'espace mémoire alloué precedemment pour le processus
