...
function Process(size, time) { //Représente un Processus
	this.size = size;
	this.timeLeft = time;
...
function Heap() { //Permet de manipuler les processus et les espaces mémoires
	this.head = null;
	this.size = 0;

	//Allouer un espace mémoire à un processus 
	//en utilisant la méthode best fit : le plus adéquat

	this.requestAllocation = async function(process) 
	{
		var instructions=document.getElementById("ins");
		blockBestFit = this.head;
		// s'assurer qu'il existe un bloc pouvant contenir le processus 
		...
		// Chercher le bloc le plus adéquat
		document.getElementById("loading").style.display="block";
		instructions.innerHTML="Chercher dans MC un block avec une taille la plus proche de "+process.size+" Ko";	
		await sleep(4000);
		...
		await sleep(4500);
		// Creation d'un nouveau bloc si nécéssaire
		if (spaceLeftover > 0) {
	...
	this.deallocateProcess = function(process) { //Libérer un espace mémoire
		process.allocatedBlock.setProcess(null);
		process.allocatedBlock = null;
	}

	this.add = function(block) //Ajouter un bloc dans la liste des blocs mémoire
	{
		if (this.head == null) 
		{
			this.head = block;
    ...
    this.fusionner = function() {//Fusionnement des bloc contigus libérés
		block=this.head;
		while (block !=null)
		{
			cpt=0;
...
var clock = setInterval(async function() {
	if (!enCours)
	{
		for (i=0; i<processes.length; i++) 
		{
			...
			{
				process.tick();
				if (process.timeLeft < 1) //Libération de l'espace mémoire alloué pour ce processus
				{
