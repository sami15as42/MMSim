	...
	function Heap() {
		...
		//Allouer un espace mémoire au processus en utilisant la méthode WorstFit: le bloc ayant la plus grande taille pouvant le contenir
		this.requestAllocation = async function(process) {
			...
			while ((blockWorstFit.size < process.size) || (!blockWorstFit.available)) {
				...
				if (blockWorstFit == null) 
				{
					if (!process.traiter)
					{
						process.traiter = true;
						document.getElementById("loading").style.display="block";
						instructions.innerHTML="Chercher dans MC le bloc ayant la plus grande taille pouvant satisfaire la demande du processus "+process.id;	
						await sleep(5000);
			...
			instructions.innerHTML="Chercher dans MC le bloc ayant la plus grande taille pouvant satisfaire la demande du processus "+process.id;	
			await sleep(4500);
	        instructions.innerHTML="Il existe un bloc dont la taille est  "+blockWorstFit.size+" Ko";
			this.repaint1(blockWorstFit);
			await sleep(3000);
			this.repaint();
			instructions.innerHTML="Allocation du bloc pour le processus et creation d'un nouveau bloc de taille  ( "+ blockWorstFit.size +" - "+process.size+" ) Ko";
			await sleep(4000);
		...
		if (!enCours)
		{
			for (i=0; i<processes.length; i++) {
				...
				if (!process.isAllocated()) {
					...
					process.tick();
					if (process.timeLeft < 1) {//Liberer l'espace mémoire alloué précedemment au processus
