...
function Heap() {
	...
	this.requestAllocation = async function(process) {
		...
		while ((blockNextFit.size < process.size) || (!blockNextFit.available)) {
			...
			if ((bblock!=null) && (blockNextFit==null)) blockNextFit = this.head;
			if (blockNextFit == bblock) 
			{
				if (!process.traiter)
				{
					process.traiter = true;
					document.getElementById("loading").style.display="block";
					instructions.innerHTML="Le dernier bloc allouée est celui de taille "+precedent.size+"Ko";
					this.repaint1(precedent);
					await sleep(3000);
					this.repaint();
					instructions.innerHTML="Chercher dans MC le premier bloc dont la taille est >= à "+process.size+" Ko en commençant la recherche à partir du dernier bloc alloué";	
					await sleep(5000);
					instructions.innerHTML="Aucun bloc n'est disponble avec une taille >= à "+process.size+" Ko";
		...
		instructions.innerHTML="Chercher dans MC le premier bloc dont la taille est >= à "+process.size+" Ko en commençant la recherche à partir du dernier bloc alloué";	
		await sleep(5000);
        instructions.innerHTML="Il existe un bloc dont la taille est  "+blockNextFit.size+" Ko";
		this.repaint1(blockNextFit);
		await sleep(3000);
		spaceLeftover = blockNextFit.size - (process.size + memControlBlockSize); // Space leftover if block was divided
		instructions.innerHTML="Allocation du bloc pour le processus et creation d'un nouveau bloc de taille  ( "+ blockNextFit.size +" - "+process.size+" ) Ko";
		await sleep(4500);
		this.repaint();
		if (spaceLeftover > 0) {
...
var clock = setInterval(async function() {
	if (!enCours)
	{
		for (i=0; i<processes.length; i++) 
		{
			...
			{
				precedent=process.allocatedBlock;
				process.tick();
				if (process.timeLeft < 1) //Liberer l'espace mémoire alloué precedemment au processus
				{
