	...
	function Heap() {
		...
		// Allouer un espace mémoire pour le processus en utilisant la méthode FirstFit : le premier qui convient
		this.requestAllocation = async function(process) {
			...
			while ((blockFirstFit.size < process.size) || (!blockFirstFit.available)) {
				...
				if (blockFirstFit == null) 
				{
					if (!process.traiter)
					{
						process.traiter = true;
						document.getElementById("loading").style.display="block";
						instructions.innerHTML="Chercher dans MC le premier bloc dont la taille est >= à "+process.size+" Ko";	
						await sleep(5000);
			...
			instructions.innerHTML="Chercher dans MC le premier bloc dont la taille est >= à "+process.size+" Ko";	
			await sleep(4500);
	        instructions.innerHTML="Il existe un bloc dont la taille est  "+blockFirstFit.size+" Ko";
			this.repaint1(blockFirstFit);
			await sleep(3000);
			this.repaint();
			spaceLeftover = blockFirstFit.size - (process.size );
			instructions.innerHTML="Allocation du bloc pour le processus et creation d'un nouveau bloc de taille  ( "+ blockFirstFit.size +" - "+process.size+" ) Ko";
			await sleep(4500);
